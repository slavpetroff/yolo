...
if [ "$MIGRATED_COUNT" -gt 0 ]; then
  echo "✓ Added $MIGRATED_COUNT new settings from defaults (run /yolo:config to see them)"
fi
```
...
```bash
PROFILE=$(jq -r '.model_profile // "quality"' .yolo-planning/config.json)
echo ""
...
echo "  Lead: $LEAD_DISPLAY | Dev: $DEV_DISPLAY | Debugger: $DEBUGGER_DISPLAY | Architect: $ARCHITECT_DISPLAY"
```
...
```bash
jq -r 'to_entries[] | select(.key | startswith("v3_") or startswith("v2_")) | "\(.key)\t\(.value)"' .yolo-planning/config.json
```

Display hint after flags: `Toggle with: /yolo:config <key> true|false`
...
```bash
CURRENT_PROFILE=$(jq -r '.model_profile // "quality"' .yolo-planning/config.json)
...
OLD_COST=$(( $(get_model_cost "$LEAD_OLD") + $(get_model_cost "$DEV_OLD") + $(get_model_cost "$DEBUGGER_OLD") + $(get_model_cost "$ARCHITECT_OLD") ))
```
...
Store selections in variables `LEAD_MODEL`, `DEV_MODEL`.
...
```bash
NEW_COST=$(( $(get_model_cost "$LEAD_MODEL") + $(get_model_cost "$DEV_MODEL") + $(get_model_cost "$DEBUGGER_MODEL") + $(get_model_cost "$ARCHITECT_MODEL") ))
...
# Get current profile
OLD_PROFILE=$(jq -r '.model_profile // "quality"' .yolo-planning/config.json)
...
# Validate agent
case "$AGENT" in
  lead|dev|debugger|architect)
    # Valid
    ;;
  *)
    echo "⚠ Unknown agent '$AGENT'. Valid: lead, dev, debugger, architect"
    exit 0
    ;;
esac
...
| agent_max_turns | object | per-agent turns (number), 0/false = unlimited | architect=30, debugger=80, lead=50, dev=75 |
