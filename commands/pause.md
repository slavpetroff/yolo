...
1. **Resolve paths:** If the active milestone above is blank/empty or says "No active milestone", use `.yolo-planning/RESUME.md`. Otherwise treat the value as the milestone slug (trim whitespace) and use `.yolo-planning/milestones/{slug}/RESUME.md`. If the resolved path's parent directory does not exist, STOP: "Milestone directory not found. Run /yolo:init or check .yolo-planning/ACTIVE."
