...
1. **Resolve context:** Always use `.yolo-planning/STATE.md` for todos — project-level data lives at the root, not in milestone subdirectories. If `.yolo-planning/STATE.md` does not exist:
   - **ACTIVE milestone exists:** Create root STATE.md by running: `yolo persist-state .yolo-planning/milestones/{SLUG}/STATE.md .yolo-planning/STATE.md "{PROJECT_NAME}"` (read SLUG from `.yolo-planning/ACTIVE`, PROJECT_NAME from the milestone STATE.md `**Project:**` line).
