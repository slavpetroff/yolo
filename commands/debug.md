...
4. **Spawn investigation:**
    **Path A: Competing Hypotheses** (prefer_teams='always' OR (effort=high AND ambiguous)):
    ...
    - Collect pre-existing issues from all debugger responses. De-duplicate by test name and file — if multiple debuggers report the same pre-existing failure, include it only once.
