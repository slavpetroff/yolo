...
- **Never hand-edit generated agent files.** Files with `<!-- GENERATED by generate-agent.sh -->` marker must be modified via templates (`agents/templates/`) or overlays (`agents/overlays/`), then regenerated with `scripts/regenerate-agents.sh --force`.
...
- Agent files (yolo-*.md) are generated from templates + overlays; never hand-edit generated agents [patterns]
