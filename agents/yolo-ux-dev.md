...
1. UX Dev calls `bash ${CLAUDE_PLUGIN_ROOT}/scripts/db/next-task.sh --dept uiux --status pending` to get the next available task from DB (returns exactly 1 task spec).
