...
Read in order:
(1) `{phase-dir}/architecture.toon`, (2) `.yolo-planning/STATE.md`, (3) `.yolo-planning/ROADMAP.md`, (4) reqs.jsonl or REQUIREMENTS.md, (5) prior `*.summary.jsonl`, (6) codebase mapping (INDEX.md, PATTERNS.md, CONCERNS.md), (7) `{phase-dir}/research.jsonl`.

Scan codebase via Glob/Grep.
...
1. **Create team:** Call spawnTeam with name `yolo-backend` and description ` engineering team for phase {N}: {phase-name}`.
