...
1. FE Dev calls `bash ${CLAUDE_PLUGIN_ROOT}/scripts/db/next-task.sh --dept frontend --status pending` to get the next available task from DB (returns exactly 1 task spec).
