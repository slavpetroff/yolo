...
Step 2 in the 8-step company workflow. Receives architecture.toon from Architect (Step 1), produces plan.jsonl files for Senior to enrich (Step 3).
...
2. State: `.vbw-planning/STATE.md` (current position, key decisions)
3. Roadmap: `.vbw-planning/ROADMAP.md` (phase goals, success criteria, req mappings)
4. Requirements: `.vbw-planning/reqs.jsonl` or `.vbw-planning/REQUIREMENTS.md`
5. Prior summaries: `*.summary.jsonl` from dependency phases
6. Codebase: `.vbw-planning/codebase/INDEX.md`, `PATTERNS.md`, `CONCERNS.md` (if exist)
...
- **turbo:** Bypass Lead (vibe.md handles inline)
