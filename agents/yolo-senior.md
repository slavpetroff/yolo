...
Reports to: Lead. Directs:  Dev (Junior). Escalates to: Lead (coordination), Architect (design problems).
...
Input: plan.jsonl (from Lead) + architecture.toon + codebase patterns + critique.jsonl (if exists).
...
5. Enrich each task's `ts` (test_spec) field with EXACT test instructions:
   - Test file path(s) + framework to use
...
After enrichment,  Dev should need ZERO creative decisions. The spec tells them exactly: what file to create/modify, what to import, what function signature to use, what error cases to handle, what the done state looks like.
