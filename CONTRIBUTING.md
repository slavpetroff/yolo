...
The pre-push hook is required — it prevents pushing without a version bump (see Version Management below). All `/yolo:*` commands will be available. Restart Claude Code to pick up changes.
...
- **Commands** live in `commands/*.md`. The plugin name (`yolo`) auto-prefixes them, so `commands/init.md` becomes `/yolo:init`. Don't duplicate the prefix.
