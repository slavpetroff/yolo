...
# verify-go.sh — Automated verification of vibe command consolidation (Plan 03-01)
#
# Checks all 25 requirements (REQ-01 through REQ-25) across 6 groups.
# Read-only: never modifies any files.
#
# Usage: bash scripts/verify-go.sh
# Exit: 0 if all pass, 1 if any fail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"

VIBE="$ROOT/commands/go.md"
PROTOCOL="$ROOT/references/execute-protocol.md"
COMMANDS_DIR="$ROOT/commands"
CLAUDE_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}"
GLOBAL_MIRROR="$CLAUDE_DIR/commands/yolo"
...
check "REQ-01" "go.md contains planning_dir_exists" grep -q "planning_dir_exists" "$VIBE"
check "REQ-01" "go.md contains phase_count=0" grep -q "phase_count=0" "$VIBE"
check "REQ-01" "go.md contains next_phase_state" grep -q "next_phase_state" "$VIBE"

# REQ-02: NL intent parsing section
check "REQ-02" "go.md has Natural language intent section" grep -q "Natural language intent" "$VIBE"
check "REQ-02" "go.md has interpret user intent" grep -q "interpret user intent" "$VIBE"

# REQ-03: Flags map to modes
check "REQ-03" "go.md maps --plan to Plan mode" grep -q "\-\-plan.*Plan mode" "$VIBE"
check "REQ-03" "go.md maps --execute to Execute mode" grep -q "\-\-execute.*Execute mode" "$VIBE"
check "REQ-03" "go.md maps --discuss to Discuss mode" grep -q "\-\-discuss.*Discuss mode" "$VIBE"

# REQ-04: Confirmation gate via AskUserQuestion
check "REQ-04" "go.md references AskUserQuestion" grep -q "AskUserQuestion" "$VIBE"

# REQ-05: --yolo skip behavior
check "REQ-05" "go.md describes --yolo flag" grep -q "\-\-yolo" "$VIBE"
check "REQ-05" "go.md describes --yolo skipping confirmations" grep -q "skip.*confirmation" "$VIBE"
...
check "REQ-17" "go.md Execute mode reads execute-protocol.md" grep -q "Read.*execute-protocol" "$VIBE"
...
# REQ-20: Positive checks — key files reference /yolo:go
check "REQ-20" "suggest-next.sh references /yolo:go" grep -q "/yolo:go" "$SUGGEST"
check "REQ-20" "help.md references /yolo:go" grep -q "/yolo:go" "$HELP"
...
check_absent "REQ-21" "go.md has no regex patterns" grep -q "regex" "$VIBE"
check_absent "REQ-21" "go.md has no import statements" grep -q "^import " "$VIBE"
check "REQ-21" "go.md has keyword-based intent matching" grep -q "keywords" "$VIBE"

# REQ-22: Ambiguous intents handled
check "REQ-22" "go.md handles ambiguous intents" grep -q "Ambiguous" "$VIBE"
check "REQ-22" "go.md offers 2-3 options for ambiguity" grep -q "2-3.*options" "$VIBE"
...
check "REQ-23" "go.md has >= 9 mode flags (found $FLAG_COUNT)" test "$FLAG_COUNT" -ge 9

# REQ-24: Behavior modifiers present
check "REQ-24" "go.md has --effort modifier" grep -q "\-\-effort" "$VIBE"
check "REQ-24" "go.md has --skip-qa modifier" grep -q "\-\-skip-qa" "$VIBE"
check "REQ-24" "go.md has --skip-audit modifier" grep -q "\-\-skip-audit" "$VIBE"
check "REQ-24" "go.md has --plan=NN modifier" grep -q "\-\-plan=NN" "$VIBE"

# REQ-25: Bare integer support
check "REQ-25" "go.md documents bare integer support" grep -qi "bare integer" "$VIBE"
check "REQ-25" "go.md bare integer targets phase N" grep -q "phase N" "$VIBE"
