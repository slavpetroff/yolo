...
case "$TYPE" in
  critique)
    ...
    sql_exec "$DB" "INSERT INTO critique (id, cat, sev, q, ctx, sug, st, cf, rd, phase)
  ...
  research)
    ...
    sql_exec "$DB" "INSERT INTO research (q, src, finding, conf, dt, rel, brief_for, mode, priority, ra, rt, phase)
    ...
    sql_exec "$DB" "INSERT INTO research_fts (rowid, q, finding, conf, phase)
  ...
  decisions)
    ...
    sql_exec "$DB" "INSERT INTO decisions (ts, agent, task, dec, reason, alts, phase)
      VALUES ('$(esc "$ts")','$(esc "$agent")','$(esc "$task")','$(esc "$dec")','$(esc "$reason")','$(esc "$alts")','$(esc "$PHASE")');"
    # Update FTS index
    rowid=$(sql_query "$DB" "SELECT last_insert_rowid();")
    sql_exec "$DB" "INSERT INTO decisions_fts (rowid, dec, reason, agent, phase)
  ...
  escalation)
    ...
    sql_exec "$DB" "INSERT INTO escalation (id, dt, agent, reason, sb, tgt, sev, st, res, phase)
  ...
  gaps)
    ...
    sql_exec "$DB" "INSERT INTO gaps (id, sev, \"desc\", exp, act, st, res, phase)
      VALUES ('$(esc "$id")','$(esc "$sev")','$(esc "$desc")','$(esc "$exp")','$(esc "$act")','${st:-open}','$(esc "$res")','$(esc "$PHASE")');"
    # Update FTS index
    rowid=$(sql_query "$DB" "SELECT last_insert_rowid();")
    sql_exec "$DB" "INSERT INTO gaps_fts (rowid, \"desc\", exp, act, res, phase)
  ...
  code-review-comment)
    ...
    sql_exec "$DB" "INSERT INTO code_review (plan, r, tdd, cycle, dt, sg_reviewed, sg_promoted, phase)
  ...
  security-finding)
    ...
    sql_exec "$DB" "INSERT INTO security_audit (r, findings, critical, dt, phase)
  ...
  qa-gate-result)
    ...
    sql_exec "$DB" "INSERT INTO qa_gate_results (gl, r, plan, task, tst, dur, f, mh, dt, phase)
