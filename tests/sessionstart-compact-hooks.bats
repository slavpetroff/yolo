...
@test "post-compact: restores role-matched snapshot and infers pending plan" {
  cd "$TEST_TEMP_DIR"
  jq '.v3_snapshot_resume = true' .yolo-planning/config.json > .yolo-planning/config.json.tmp && \
    mv .yolo-planning/config.json.tmp .yolo-planning/config.json

  cat > .yolo-planning/.execution-state.json <<'STATE'
{"phase":1,"status":"running"}
STATE

  mkdir -p .yolo-planning/.snapshots .yolo-planning/.events
  cat > .yolo-planning/.snapshots/1-20260101T000000.json <<'SNAP1'
{"snapshot_ts":"20260101T000000","phase":1,"agent_role":"yolo-qa","execution_state":{"status":"running","plans":[{"id":"01-02","status":"pending"}]},"recent_commits":[]}
SNAP1
  cat > .yolo-planning/.snapshots/1-20260101T000001.json <<'SNAP2'
{"snapshot_ts":"20260101T000001","phase":1,"agent_role":"yolo-dev","execution_state":{"status":"running","plans":[{"id":"01-01","status":"complete"},{"id":"01-02","status":"pending"}]},"recent_commits":[]}
SNAP2
...
EVENTS
  ...
  run bash -c 'echo "{\"agent_name\":\"yolo-dev\"}" | bash "$1"' _ "$SCRIPTS_DIR/post-compact.sh"
  [ "$status" -eq 0 ]
...
@test "hook-wrapper: prefers cache over CLAUDE_PLUGIN_ROOT when both exist" {
  ...
  mkdir -p "$TEST_TEMP_DIR/.claude/plugins/cache/yolo-marketplace/yolo/1.0.0/scripts"
  cp "$SCRIPTS_DIR/hook-wrapper.sh" "$TEST_TEMP_DIR/.claude/plugins/cache/yolo-marketplace/yolo/1.0.0/scripts/"
  cp "$SCRIPTS_DIR/resolve-claude-dir.sh" "$TEST_TEMP_DIR/.claude/plugins/cache/yolo-marketplace/yolo/1.0.0/scripts/"
  cat > "$TEST_TEMP_DIR/.claude/plugins/cache/yolo-marketplace/yolo/1.0.0/scripts/mock-source.sh" <<'MOCK'
#!/bin/bash
echo "FROM_CACHE"
exit 0
MOCK
  chmod +x "$TEST_TEMP_DIR/.claude/plugins/cache/yolo-marketplace/yolo/1.0.0/scripts/mock-source.sh"
...
MOCK
  ...
  # Cache should win
  run bash -c "export HOME='$TEST_TEMP_DIR'; export CLAUDE_PLUGIN_ROOT='$TEST_TEMP_DIR/fake-plugin'; bash '$TEST_TEMP_DIR/.claude/plugins/cache/yolo-marketplace/yolo/1.0.0/scripts/hook-wrapper.sh' mock-source.sh"
  [ "$status" -eq 0 ]
  [[ "$output" == *"FROM_CACHE"* ]]
}
...
@test "security-filter: exit 2 preserved through hook-wrapper logic" {
  ...
  mkdir -p "$TEST_TEMP_DIR/.claude/plugins/cache/yolo-marketplace/yolo/1.0.0/scripts"
  cp "$SCRIPTS_DIR/hook-wrapper.sh" "$TEST_TEMP_DIR/.claude/plugins/cache/yolo-marketplace/yolo/1.0.0/scripts/"
  cp "$SCRIPTS_DIR/security-filter.sh" "$TEST_TEMP_DIR/.claude/plugins/cache/yolo-marketplace/yolo/1.0.0/scripts/"
  cp "$SCRIPTS_DIR/resolve-claude-dir.sh" "$TEST_TEMP_DIR/.claude/plugins/cache/yolo-marketplace/yolo/1.0.0/scripts/"

  INPUT='{"tool_name":"Read","tool_input":{"file_path":".env"}}'
  run bash -c "export HOME='$TEST_TEMP_DIR'; echo '$INPUT' | bash '$TEST_TEMP_DIR/.claude/plugins/cache/yolo-marketplace/yolo/1.0.0/scripts/hook-wrapper.sh' security-filter.sh"
  [ "$status" -eq 2 ]
}
