...
setup() {
  ...
  # Create a .yolo-session marker (should be preserved by session-stop)
  echo "test-session" > "$TEST_TEMP_DIR/.yolo-planning/.yolo-session"

  # Clean stale PID tracker lock from previous interrupted runs
  rmdir /tmp/yolo-agent-pid-lock 2>/dev/null || true
}
...
@test "two agent-starts increment count to 2" {
  ...
  simulate_agent_start "yolo-dev" "$pid1"
  simulate_agent_start "yolo-qa" "$pid2"
...
@test "agent-start registers PID — agent-stop unregisters it" {
  ...
  [ -f ".yolo-planning/.agent-pids" ]
  run grep "^${pid}$" ".yolo-planning/.agent-pids"
  [ "$status" -eq 0 ]
...
@test "session-stop removes transient markers" {
  ...
  echo "dev" > ".yolo-planning/.active-agent"
  echo "2" > ".yolo-planning/.active-agent-count"
  mkdir -p ".yolo-planning/.active-agent-count.lock"
  echo "12345 %1" > ".yolo-planning/.agent-panes"

  simulate_session_stop

  [ ! -f ".yolo-planning/.active-agent" ]
  [ ! -f ".yolo-planning/.active-agent-count" ]
  [ ! -d ".yolo-planning/.active-agent-count.lock" ]
  [ ! -f ".yolo-planning/.agent-panes" ]
}

@test "session-stop removes .task-verify-seen (circuit breaker state)" {
  cd "$TEST_TEMP_DIR"
  echo "abc123hash" > ".yolo-planning/.task-verify-seen"
...
@test "session-stop preserves .yolo-session" {
  cd "$TEST_TEMP_DIR"

  simulate_session_stop

  [ -f ".yolo-planning/.yolo-session" ]
  run cat ".yolo-planning/.yolo-session"
  [ "$output" = "test-session" ]
}
...
@test "session-stop appends to session log" {
  ...
  [ -f ".yolo-planning/.session-log.jsonl" ]
  run jq -r '.model' ".yolo-planning/.session-log.jsonl"
  [ "$output" = "test" ]
}

@test "session-stop persists and removes cost ledger" {
  cd "$TEST_TEMP_DIR"
  echo '{"lead":0.05,"dev":0.10}' > ".yolo-planning/.cost-ledger.json"

  simulate_session_stop

  # Cost ledger should be gone
  [ ! -f ".yolo-planning/.cost-ledger.json" ]

  # Session log should contain the cost_summary entry (jq slurp handles multi-line JSON)
  run jq -s '[.[] | select(.type == "cost_summary")] | length' ".yolo-planning/.session-log.jsonl"
  [ "$output" = "1" ]
}
...
@test "full chain: multi-agent start → task-verify blocks → circuit breaker → stops → session cleanup" {
  ...
  pid2=$(next_fake_pid)
  simulate_agent_start "yolo-dev" "$pid1"
  simulate_agent_start "yolo-dev" "$pid2"
