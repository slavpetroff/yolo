...
# Version bumping utility: updates 4 version files
...
setup() {
  ...
  # Create mock project structure with all 4 version files
  ...
  echo '{"plugins":[{"version":"1.2.3","name":"yolo"}]}' > "$MOCK_ROOT/marketplace.json"
...
@test "--verify passes when all 4 files are in sync" {
  run bash -c "cd '$MOCK_ROOT' && bash scripts/bump-version.sh --verify"
  assert_success
  assert_output --partial "All 4 version files are in sync"
}
...
# --- 4. Updates all 4 files ---

@test "updates all 4 version files consistently" {
  run bash -c "cd '$MOCK_ROOT' && PATH='$TEST_WORKDIR/bin:$PATH' bash scripts/bump-version.sh"
  assert_success
  ...
  assert_output "1.2.4"
  run jq -r '.plugins[0].version' "$MOCK_ROOT/marketplace.json"
  assert_output "1.2.4"
}
...
# --- 6. --verify shows all 4 file paths ---

@test "--verify output shows all 4 file versions" {
  run bash -c "cd '$MOCK_ROOT' && bash scripts/bump-version.sh --verify"
  assert_success
  assert_output --partial "VERSION"
  assert_output --partial ".claude-plugin/plugin.json"
  assert_output --partial ".claude-plugin/marketplace.json"
  assert_output --partial "marketplace.json"
}
